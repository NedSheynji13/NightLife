...
{
    #region Variables
    public GameObject weakLight, strongLight;
    ...
    void Update()
    {
        if (Input.GetMouseButton(0))
            Instantiate(weakLight, new Vector3(Spawn().x, 5, Spawn().z), Quaternion.identity);
        else if (Input.GetMouseButtonDown(1) && isRunning == false)
